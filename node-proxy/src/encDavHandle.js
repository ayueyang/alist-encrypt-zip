...
const handle = async (ctx, next) => {
  ...
  if ('GET,PUT,DELETE'.includes(request.method.toLocaleUpperCase()) && passwdInfo && passwdInfo.encName) {
    ...
    request.url = path.dirname(request.url) + '/' + realName
    request.urlAddr = path.dirname(request.urlAddr) + '/' + realName
    if (isWinZipAesEncType(passwdInfo.encType) && request.method.toLocaleUpperCase() !== 'PUT') {
      await next()
      return
    }
    ...
    const fileDetail = isWinZipAesEncType(passwdInfo.encType)
      ? {
          path: path.dirname(url) + '/' + realName,
          name: realName,
          is_dir: false,
          size: WinZipAesZip.packageSize(contentLength * 1, { originalName: fileName }),
        }
      : { path: url, name: fileName, is_dir: false, size: contentLength }
