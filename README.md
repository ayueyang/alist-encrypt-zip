...
本分支新增 `zip` 加密方式，适合希望网盘里保存为 `.zip` 压缩包，同时又能通过 alist-encrypt 在线播放的场景。

- `compatible` 模式会生成真正的标准 ZIP 包，使用 ZipCrypto + store 存储方式。上传后原始 AList/网盘里看到的是加密后的 `密文.原后缀.zip` 文件；通过 alist-encrypt 代理访问时显示原文件名，并支持在线播放。
- 这个模式的重点是兼容普通解压软件：7-Zip、WinRAR、RAR、Bandizip 等常规 ZIP/RAR/7z 解压软件都可以直接打开这个 `.zip`，输入同一个密码后解压出内容正常、可播放的视频文件。
- ZIP 内部不会保存明文原文件名，解压出来的文件名可能是通用名或密文名，但文件内容是解密后的正常视频。
- `fake` 模式只是伪装成 ZIP 外壳，内部继续使用流加密，适合强度优先和代理在线播放；普通解压软件不能直接解密 `fake` 模式文件。
- `compatible` 模式为了兼容常规解压软件使用 ZipCrypto，兼容性强但加密强度弱于现代 AES；需要更强加密时建议使用 `fake` 模式。
